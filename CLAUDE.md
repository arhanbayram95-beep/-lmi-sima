...
* **Frontend:** React Native + TypeScript, Expo (blank TS template). State via `Zustand`. Camera via `expo-camera`. Haptics via `expo-haptics`. Audio via `expo-av`. Share cards via `react-native-view-shot`. Styling strictly via tokens in `frontend/src/ui/theme.ts` (Crimson `#9E2941`, Champagne Gold `#EBC983`, Dark Obsidian `#1A050B`).
