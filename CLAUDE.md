...
* The system prompt and all generated copy must stay in a warm, constructive, modern "vibe reading" register — **NO clinical, diagnostic, or psychiatric language; NO negative or trust-undermining character claims.** Never overridden, regardless of any tone/theming direction below.
* **Oracle/Arcana/mystic-fantasy vocabulary is authorized (2026-08-15 product decision).** This reverses the prior blanket "no oracle" rule below — the product owner explicitly and deliberately overrode it after it was flagged twice (once declined outright for a "Mystical Tarot Deck" request, then re-flagged and authorized for the "deep master card" reveal-screen redesign that followed it same-day). "Oracle," "arcana," "aura," "totem," "sacred," and "legend" are now the reveal screen's house style — see `backend/src/services/systemPrompt.ts`'s `TONE_GUIDANCE` and `PROJECT_SPEC.md` §1's redesign notes for the full authorized scope. Still off-limits, not part of what was authorized: any *specific* real-world religious, ethnic, or historical tradition (no medieval European court, no Ottoman, no real culture's actual practices — the mysticism stays generic and invented), and any flavor stat presented as if backed by real data that doesn't exist (a percentage or "1 in N" number must always come with a real generated explanation, never a bare claim like "correlation across geometric facial datasets" — see `PROJECT_SPEC.md`'s 2026-08-15 metric-honesty note).
